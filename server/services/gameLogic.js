 ...
 * @param {String} currentPlayer - 当前玩家颜色
 * @returns {Object} - { isValid, newBoard, capturedPiece }
 */
const validateMove = (board, fromRow, fromCol, toRow, toCol, currentPlayer) => {
  // 检查起始位置是否有棋子
  ...
  // 检查是否出现"将帅对面"的情况
  if (isGeneralsFacing(newBoard)) {
    return { isValid: false };
  }
  
  return { isValid: true, newBoard, capturedPiece };
};
 ...
 */
const isValidMove = (piece, board, fromRow, fromCol, toRow, toCol) => {
  // 目标位置已经有己方棋子
 ...
 * 检查将帅是否面对面（长将情况）
 * @param {Array} board - 棋盘状态
 * @returns {Boolean} - 是否将帅对面
 */
const isGeneralsFacing = (board) => {
  let redGeneralCol = -1;
 ...
 */
const checkGameEnd = (board, currentTurn) => {
          ...
          for (let toRow = 0; toRow < 10; toRow++) {
            for (let toCol = 0; toCol < 9; toCol++) {
              if (isValidMove(piece, board, fromRow, fromCol, toRow, toCol)) {
                ...
                // 检查移动后是否仍被将军
                const stillInCheck = isCheck(newBoard, opponentColor);
